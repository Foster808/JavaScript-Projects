...
// this function will roll for random number twice, on for
...
function rollForTurn() {
    ...
    diceRoll(); // play dice sound during the game roll for turn
...
// this function style the game buttons while they are disable
...
function avatarPlaced() {
    ...
    if (activePlayer == "Player 1") { //once acitve player selects a square change the active player
...
// this fucntion will get the array of the current board
...
function checkForWinCon() {
    ...
    info = info.split(','); //separe the string by commas into an array
    ...
    // call this following array of funtions to check for any of the possible win cons
...
function winner(winDetected,winCon) {
    if (winDetected == "win") {
        ...
        startEnabled(btn); //enable the start button since the game is now stopped
        var btn = document.getElementById('btnStop');
        btnDisabled(btn); //disable the stop button sinc ethe game is now stopped
...
//this function will make the winning square light up in celebration
...
//--------------------------------------------------------------------------
//checking for wincon square 012
function checkWinCon1(info,squareArray) {
    ...
    //avatars and copare all 3 for win cons
    for (var i in info) {
        if (info[i].charAt(0) == "0") {
            var match0Avatar = info[i].charAt(1); //only interest in recording the avatar
        }
    ...
    winner(winDetected,winCon1); //winCon1 is teh array of win combinations
}

//checking for wincon square 345
...
//checking for wincon square 642
...
//this function will perfomr the animation for the X avatar
